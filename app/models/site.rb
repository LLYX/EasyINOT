class Site < ActiveRecord::Base
    ...
    def self.import(file)
      CSV.foreach(file.path, headers: true, :encoding => 'windows-1252') do |row|
        ...
        site.departments = !row['departments'].nil? ? Department.where(english_name: row['departments'].split(',')) : Department.where(english_name: '')
        site.groups = !row['groups'].nil? ? Group.where(english_name: row['groups'].split(',')) : Group.where(english_name: '')
        site.save!
      end
    end
end
